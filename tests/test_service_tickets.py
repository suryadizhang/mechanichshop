...
class TestServiceTicketRoutes(BaseTestCase):
    ...
    def test_create_service_ticket_nonexistent_customer(self):
        ...
        # Service tickets can be created without valid customer (walk-in customers)
        self.assertEqual(response.status_code, 201)
        data = response.get_json()
        self.assertEqual(data['customer_id'], 999)
        self.assertIsNone(data['customer'])  # Customer relationship will be None
    ...
    def test_assign_mechanic_to_ticket(self):
        ...
        url = f'/service-tickets/{self.ticket_id}/assign-mechanic/{self.mechanic_id}'
    ...
    def test_remove_mechanic_from_ticket(self):
        ...
        assign_url = f'/service-tickets/{self.ticket_id}/assign-mechanic/{self.mechanic_id}'
        self.client.put(assign_url)
        
        # Then remove mechanic
        remove_url = f'/service-tickets/{self.ticket_id}/remove-mechanic/{self.mechanic_id}'
    ...
    def test_remove_unassigned_mechanic(self):
        ...
        url = f'/service-tickets/{self.ticket_id}/remove-mechanic/{self.mechanic_id}'
